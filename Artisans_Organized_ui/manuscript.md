...
*   **Frontend Environment:** Flutter SDK (>=3.1.1 <4.0.0), Dart, PWA configuration assets.
*   **Web Portal:** React (18.3), Vite, Tailwind CSS.
*   **Infrastructure:** Supabase (PostgreSQL 15), Firebase Admin SDK for push notifications, Vercel for CI/CD and Edge network deployment.
...
2.  **Network Volatility & Accessibility:** Intermittent connectivity necessitated robust offline-first UI handling and optimistic state updates. Deploying the client additionally as a PWA mitigates app-store friction and storage constraints on entry-level smartphones.
